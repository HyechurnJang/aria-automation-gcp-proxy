...
def handler(context, inputs):
    gcpProxyUrl = inputs.pop('gcpProxyUrl') # get proxy url
    payload = { # check property and create payload
        'id': inputs['id'],
        'name': inputs['name'],
        'type': inputs['type'],
        'projectId': inputs['projectId'],
        'privateKeyId': inputs['privateKeyId'],
        'privateKey': context.getSecret(inputs['privateKey']), # decrypt encrypted property
        'clientEmail': inputs['clientEmail'],
        'clientId': inputs['clientId'],
        'authUri': inputs['authUri'] if 'authUri' in inputs else 'https://accounts.google.com/o/oauth2/auth', # set default
        'tokenUri': inputs['tokenUri'] if 'tokenUri' in inputs else 'https://oauth2.googleapis.com/token', # set default
        'authProviderX509CertUrl': inputs['authProviderX509CertUrl'] if 'authProviderX509CertUrl' in inputs else 'https://www.googleapis.com/oauth2/v1/certs', # set default
        'clientX509CertUrl': inputs['clientX509CertUrl']
    }
    res = requests.put(gcpProxyUrl + '/secret/' + inputs['id'], json=payload)
